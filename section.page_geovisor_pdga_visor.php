<link rel="stylesheet" type="text/css" href="./css/page_template.css" />

<div class="row h100p">
			
	<!----------------------sda------------------------>
	<div class="col-md-12 col-lg-12 section-b h100p p0 jump-posrel">

		<div id="map">
		
		
			
		</div>
		
		<div class="jus-left row jump-row" id="btn-ver-geovisor" style="max-width:200px; display: flex; left:30px;">
			<div style="width:200px;">
				<a href="#" class="black-button" id="veg-btn" target="_blank">VER EN GEOVISOR</a>
			</div>
		</div>
		<div class="jus-left row jump-row" style="position:absolute; top:20px; right:35px; max-width:350px; display: flex;">
			<div style="width:350px;">
				<span>Sitios de Observación y Medición del PDGA</span>
			</div>
		</div>
		
	</div>
	
</div>