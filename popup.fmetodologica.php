<div class="jump-window col col-xs-12 col-sm-12-col-md-4 col-lg-4 geovisor-flotant" id="popup-fmetodologica"
 data-minimizable="0">
	
	<div class="jump-window-inner p20">

		<div class="jump-window-header">
			<span id="ficha-metodologica-titulo-prev"><img src="./images/ficha-icono-2.png"></span><span id="ficha-metodologica-titulo">Ficha Metodologica</span>
			<div class="jump-window-icon-bar">
				<a href="#" class="jump-window-close" onclick="jwindow.close('popup-fmetodologica');">
					<i class="fas fa-times"></i>
				</a>
			</div>
			<div class="jump-window-icon-bar">
				<a href="#" class="jump-window-icon" onclick="jwindow.close('popup-fmetodologica');">
					<img src="./images/ficha-heart.png">
				</a>
				<a href="#" class="jump-window-icon" id="ficha-metodologica-download" onclick="jwindow.close('popup-fmetodologica');" download>
					<img src="./images/ficha-download.png">
				</a>
			</div>
			<br>
			<hr>
		</div>
		
		<div class="jump-window-body p20">
			
			<div class="row">
				
				<div class="col col-md-10 col-sm-10 col-xs-10 col-lg-10">
			
					<div id="info-fmetodologica" class="pt-10">
					
						<p id="ficha-metodologica-desc">
						
							Seleccione un Indicador para ver su descripción.
						
						</p>
					
					</div>
					
					<a href="#" class="jump-window-icon" id="ficha-metodologica-view">
					
						<img src="./images/ficha-button.png">
					
					</a>
					
				</div>
				
				<div class="col col-md-2 col-sm-2 col-xs-2 col-lg-2">
					
					<div class="indicadores-icon-bar">
					
						<p><a href="#"><img src="./images/icono-mediateca-br.png" class="tooltip" title="VER RECURSO ASOCIADO EN MEDIATECA"></a></p>
						<p><a href="#"><img src="./images/icono-geovisores-br.png" class="tooltip" title="VER RECURSO ASOCIADO EN GEOVISORES"></a></p>
						<p><a href="#"><img src="./images/icono-estadisticas-br.png" class="tooltip" title="VER RECURSO ASOCIADO MODULO ESTADISTICOS"></a></p>					
					</div>
					
				</div>
			
			</div>
			
		</div>
	
	</div>

</div>